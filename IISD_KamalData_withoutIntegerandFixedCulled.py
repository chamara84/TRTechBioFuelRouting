...
#objective function
prob+=lpSum([(2*D_FS_ij[i][j]/(1000*speed_stinger*E_stinger)*(1-theta_baler)*C_stinger/LoadMass_stinger)*B_ikjt[i][k][j][t] for i in farms for k in typesOfBiomass for j in storages for t in periods]) \
+lpSum([2*(C_trailer+C_truck)*D_S_j[j]/(1000*speed_truck*E_truck*LoadMass_truck)*R_kjt[k][j][t] for  k in typesOfBiomass for j in storages for t in periods])\
