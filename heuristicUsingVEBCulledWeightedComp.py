...
CostWeighted=numpy.ones((num_sources,num_sinks))*float('inf')
supply=numpy.zeros((num_sources,))
source_nodes=numpy.zeros((num_sources,))

for j in range(L1_distance_multiCrop.shape[0]):
    for i in range(L1_distance_multiCrop.shape[1]):
        if supply_multiCrop[j,i]>0:
            ...
            if all(numpy.isinf(CostWeighted[i])):
                CostWeighted[i] = Cost_multiCrop[j*(num_sources)+i]*supply_multiCrop[j,i]/sum(supply_multiCrop[:,i])
            else:
                CostWeighted[i] += Cost_multiCrop[j*(num_sources)+i]*supply_multiCrop[j,i]/sum(supply_multiCrop[:,i])
            source_nodes[i]=source_nodes_multiCrop[j,i]
supply=numpy.sum(supply_multiCrop,axis=0)
    
    #
    
(num_sources,num_sinks)=CostWeighted.shape
...
def findAllocationForGivenActiveStorages(testSet,prevMinSol,prevYVec,costForTotalAvailAscend,fixedCostStorage,availability,demand,B_ij,treeArrayTemp,T_min):
    ...
    for testAllocation in prevTestSet:
        sol=0
        rowChanged = []
        allocation = numpy.zeros((len(availability),len(testAllocation),))
...
percentageHarvested = [100,50,25]
demandPoints = 20
keep_rows = numpy.where(supply>0)[0]
#harvest_cost=harvest_cost_multiCrop[cropType,:]
#highway_distance=highway_distance_multiCrop
savesupply=supply[keep_rows]
Cost=Cost[numpy.ix_(keep_rows,range(num_sinks))]
(num_sources,num_sinks)=Cost.shape
x=numpy.zeros((1,demandPoints,))
yMILP=numpy.zeros((len(percentageHarvested),demandPoints,))
yvEB = numpy.zeros((len(percentageHarvested),demandPoints,))
for percentage in range(len(percentageHarvested)):
    
    supply=float(percentageHarvested[percentage])/100*savesupply
    ...
    for demand in [i*20000 for i in range(1,demandPoints+1)]:
                ...
                if sum(supply)>=demand:
                    ...
                    prob += pulp.lpSum([S[i]*xvars[(i,j)] for (i,j) in non_zero_node_pairs]) >= demand
                    prob.writeLP("mylp.lp")
                    
                    prob.solve()
                    print 'MILP time',demand,time.time()-t1,pulp.value(prob.objective)
                    
                    x[0,demand/20000-1] = demand
                    yMILP[percentage,demand/20000-1]=pulp.value(prob.objective)/demand
 ...
 #####################             #vEB algorithm      ########################################
                    ...
                    probLP += pulp.lpSum([S[i]*xvarsLP[(i,j)] for (i,j) in non_zero_node_pairs]) >= demand
                    ...
                    yvEB[percentage,demand/20000-1]=prevMinSol/demand
 ################################################################################################
...
for p in range(len(percentageHarvested)):
        plt.plot(x[0,:],yvEB[p],label='VEB percent harvested = '+str(percentageHarvested[p]))
        plt.plot(x[0,:],yMILP[p],label='MILP percent harvested = '+str(percentageHarvested[p]))
plt.title('Total Weighted Cost per Ton at Plant Gate')
plt.xlabel('Total Demand (Tons)')
plt.ylabel('Cost in $/ton')
plt.legend(loc=2)
plt.savefig('CentreportTotalWeightedCostEstimates.pdf',format='pdf')
    #
