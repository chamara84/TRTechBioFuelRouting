...
while len(satelliteStores)<= minSatStorages and expand <=5:
    ...
    for row in matchingStores:
           ...
           if len(satelliteStores)== 0:
              satelliteStores[string.join(("s",str(row[2])),"_")]=[row[0],row[1]]
              satStoreIDSet.add(row[2])
              
           minL2Distance =min([math.sqrt(math.pow(row[0]-satelliteStores[store][0],2)+math.pow(row[1]-satelliteStores[store][1],2)) for store in satelliteStores]) 
...
satelliteStores =dict(satelliteStores)
...
DistSatToPlant = dict()
plantCoord = [617290.1441,5443508.364] #node 150
for source in satelliteStores:
    DistSatToPlant[source] = math.sqrt(math.pow(satelliteStores[source][0]-plantCoord[0],2) + math.pow(satelliteStores[source][1]-plantCoord[1],2))
...
#objective function
prob+=lpSum([(2*D_FS_ij[i][j]/(speed_stinger*E_stinger)*(1-theta_baler)*C_stinger/LoadMass_stinger)*B_ikjt[i][k][j][t] for i in farms for k in typesOfBiomass for j in storages for t in periods]) \
+lpSum([2*(C_trailer+C_truck)*D_S_j[j]/(speed_truck*E_truck*LoadMass_truck)*R_kjt[k][j][t] for  k in typesOfBiomass for j in storages for t in periods])
