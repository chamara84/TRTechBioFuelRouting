...
for j in range(L1_distance_multiCrop.shape[0]):
    for i in range(Cost_multiCrop.shape[1]):
        ...
        Cost_multiCrop[j,i,:]=harvest_cost_multiCrop[j,i]
...
for cropType in range(num_crops):
    ...
    plt.title('Harvest Cost of '+ cropnames[str(cropType)]+' per Ton at Plant Gate')
    plt.xlabel('Total Demand (Tons)')
    plt.ylabel('Cost in $/ton')
    plt.legend(loc=4)
    plt.savefig('Centreport harvest Cost Estimates '+ cropnames[str(cropType)]+'.png')
    #
