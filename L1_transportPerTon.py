...
for j in range(L1_distance_multiCrop.shape[0]):
    for i in range(Cost_multiCrop.shape[1]):
        ...
        Cost_multiCrop[j,i,:]=L1_distance_multiCrop[j,i]/1000.*L1_weight
        
(num_crops,num_sources,num_sinks)=Cost_multiCrop.shape
for cropType in range(num_crops):
    ...
    for percent_harvested in [100, 50, 25]:
            ...
            if cropnames[str(cropType)] in ['grass','beans','flax']:
                step = 5000
            else:
                step= 20000
    ...
    plt.title('L1 Transport Cost of '+ cropnames[str(cropType)]+' per Ton at Plant Gate')
    plt.xlabel('Total Demand (Tons)')
    plt.ylabel('Cost in $/ton')
    plt.legend(loc=4)
    plt.savefig('Centreport L1 Cost Estimates '+ cropnames[str(cropType)]+'.png')
    #
