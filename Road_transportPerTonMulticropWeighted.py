...
sink_node_order=[]
sink_node_order_set = set()
for i in range(num_sources):
        min_element=numpy.argmin(Cost1[i])
        if min_element not in sink_node_order_set:
            sink_node_order+=[min_element]
            sink_node_order_set.add(min_element)
other_sink_nodes=list(set(range(num_sinks)).difference(sink_node_order))
