...
def findAllocationForGivenActiveStorages(testSet,prevMinSol,costForTotalAvailAscend,fixedCostStorage,availability,demand,B_ij,treeArrayTemp,T_min,costForTotalAvailDict,availabilityDict,sourceLabel,destLabel):
    ...
    for testAllocation in prevTestSet:
        ...
        sol=0
        storageSelDict =  pulp.makeDict([destLabel],storageSelDict,0)
        ...
        if prob.status==1:
            ...
            sol = float('inf')
        print "solLP:", sol
        print "yjLP:", [storageSelDict[j] for j in destLabel]   
...
while len(testSet)>0:
    bestStorageAllocation,prevMinSol,bestSourceToDestAlloc =  findAllocationForGivenActiveStorages(testSet,prevMinSol,costPerTonArrayAscend,fixedCostStorage,availability,demand,B_ij,treeArray,T_min,costForTotalAvailDict,availabilityDict,sourceLabel,destLabel)     
    testSet.clear()
