...
for index in range(numFarms):
  storeCap.append(np.random.randint(215,1935))
...
Distance_ij_raw  = []
with open('/home/chamara/Dropbox/TRTech/Chamara/Storage2StorageDistance.csv', 'rb') as csvfile:
...
#the number of trips from 
Trips_j = LpVariable.dicts("NumTripsFromStorejToPlant",storages,0,None,LpInteger)

B_bar_j =  LpVariable.dicts("RouteStore_max",storages,0,None,LpContinuous)


#objective function
prob+=lpSum([((C_baler+C_tractor)/(CAP_baler*E_baler)+((1-theta_baler)*C_twine)/BaleMass+theta_baler*P_k[k]+(1-theta_baler)/BaleMass*(L_stinger/E_stinger_load+U_stinger/E_stinger_unload)*C_stinger+2*D_FS_ij[i][j]/(speed_stinger*E_stinger)*(1-theta_baler)*C_stinger/LoadMass_stinger+(1-theta_baler)*theta_stinger*(P_k[k]+(C_baler+C_tractor)/(CAP_baler*E_baler))+(1-theta_baler)*theta_stinger*C_twine/BaleMass+(1-theta_baler)*(1-theta_stinger)*theta_storage*(P_k[k]+(C_baler+C_tractor)/(CAP_baler*E_baler)+C_twine/BaleMass+C_stinger/BaleMass*(L_stinger/E_stinger_load+U_stinger/E_stinger_unload)+2*C_stinger*D_FS_ij[i][j]/(speed_stinger*E_stinger*LoadMass_stinger)))*B_ikjt[i][k][j][t] for i in farms for k in typesOfBiomass for j in storages for t in periods]) + lpSum([C_F_loader*Z_j[j] for j in storages]) + lpSum([(C_V_loader*L_loader/(LoadMass_loader*E_loader)+theta_loader*(P_k[k]+(C_baler+C_tractor)/(CAP_baler*E_baler)+C_twine/BaleMass+C_stinger*(L_stinger/E_stinger_load+U_stinger/E_stinger_unload)/BaleMass+2*C_stinger*D_FS_ij[i][j]/(speed_stinger*E_stinger*LoadMass_stinger)+C_storage)+C_V_loader*(L_loader+U_loader)/(LoadMass_loader*E_loader) +(1-theta_loader)*theta_truck*(P_k[k]+(C_baler+C_tractor)/(CAP_baler*E_baler)+C_twine/BaleMass+(L_stinger/E_stinger_load+U_stinger/E_stinger_unload)/BaleMass+2*C_stinger*D_FS_ij[i][j]/(speed_stinger*E_stinger*LoadMass_stinger)+C_storage+L_loader*C_V_loader/(LoadMass_loader*E_loader)))*R_kjt[k][j][t] for  k in typesOfBiomass for j in storages for t in periods]) + lpSum([C_storage*B_bar_j[j] for j in storages])+lpSum([2*(C_trailer+C_truck)*D_S_j[j]/(speed_truck)*Trips_j[j] for j in storages]) #filled upto 5.21
...
#constraint to force integrality
for j in storages:
  for t in periods:
    prob+= Trips_j[j] - lpSum([R_kjt[k][j][t] for k in typesOfBiomass])>= 0 
