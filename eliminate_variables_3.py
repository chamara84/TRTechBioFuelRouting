...
for cropType in range(num_crops):
    ...
    for percent_harvested in [100, 50, 25]:
            ...
            for demand in [i*step for i in range(1,3)]:
    ...
    plt.title('Transport Cost of '+ cropnames[str(cropType)]+' per Ton at Plant Gate')
    plt.xlabel('Total Demand (Tons)')
    plt.ylabel('Cost in $/ton')
    plt.legend(loc=4)
    plt.savefig('Centreport Cost Estimates '+ cropnames[str(cropType)]+'.png')
    #
