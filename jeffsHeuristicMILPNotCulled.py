...
costForTotalAvail = [[costPerTon[i][j]*availability[i] for j in range(costPerTon.shape[1])] for i in range(len(availability))]
sourceLabel = [string.join(('s',str(index)),'_') for index in range(len(availability))]
destLabel =  [string.join(('d',str(index)),'_') for index in range(costPerTon.shape[1])]
